function Get-GPPP {
    ...
    [CmdletBinding()]
        ...
        try {
                ...
                $Xml.GetElementsByTagName('Properties') | ForEach-Object {
                    if ($_.cpassword) {
                        ...
                        if ($Cpassword -and ($Cpassword -ne '')) {
                           $DecryptedPassword = Get-DCP $Cpassword
                           $Password = $DecryptedPassword
                           Write-Verbose "[Get-GPPIF] Decrypted password in '$File'"
                        }
                        ...
                        try {
                            ...
                            Write-Verbose "[Get-GPPIF] Unable to retrieve ParentNode.changed for '$File'"
                        }
                        ...
                        try {
                            ...
                            Write-Verbose "[Get-GPPIF] Unable to retrieve ParentNode.ParentNode.LocalName for '$File'"
                        }
            ...
            Write-Warning "[Get-GPPIF] Error parsing file '$File' : $_"
        }
    }
    function Get-DT {
        [CmdletBinding()]
        ...
        if (Test-Connection -Count 1 -Quiet -ComputerName $Domain) {
            try {
                ...
                Write-Verbose "[Get-DT] Error contacting domain '$Domain' : $_"
            }
            ...
            try {
                ...
                Write-Verbose "[Get-DT] Error contacting forest '$Domain' (domain may not be a forest object) : $_"
            }
        }
    }
    function Get-DTM {
        ...
        try {
            ...
            Write-Warning "[Get-DTM] Error enumerating current domain: $_"
        }
        ...
        if ($CurrentDomain -and $CurrentDomain -ne '') {
            ...
            while($Domains.Count -ne 0) {
                ...
                if ($Domain -and ($Domain.Trim() -ne '') -and (-not $SeenDomains.ContainsKey($Domain))) {
                    ...
                    Write-Verbose "[Get-DTM] Enumerating trusts for domain: '$Domain'"
                    $Null = $SeenDomains.Add($Domain, '')

                    try {
                        Get-DT -Domain $Domain | Sort-Object -Unique | ForEach-Object {
    ...
    try {
        ...
        Write-Verbose '[Get-GPPP] Searching local host for any cached GPP files'
        ...
        if ($SearchForest) {
            Write-Verbose '[Get-GPPP] Searching for all reachable trusts'
            $Domains += Get-DTM
        }
        else {
            if ($Server) {
        ...
        ForEach ($Domain in $Domains) {
            Write-Verbose "[Get-GPPP] Searching \\$Domain\SYSVOL\*\Policies. This could take a while."
        ...
        if ( -not $XMLFiles ) { throw '[Get-GPPP] No preference files found.' }

        Write-Verbose "[Get-GPPP] Found $($XMLFiles | Measure-Object | Select-Object -ExpandProperty Count) files that could contain passwords."

        ForEach ($File in $XMLFiles) {
            $Result = (Get-GPPIF $File.Fullname)
            $Result
        }
    }
